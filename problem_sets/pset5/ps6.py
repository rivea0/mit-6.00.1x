...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in lc:
            char = chr(ord(i) + shift % length)
            if char not in lc:
                x = length
                char = chr(ord(char) - length) 
                x += 1
                shift_dict[i] = char
                shift_dict[i.upper()] = char.upper()
            else:
                shift_dict[i] = char
                shift_dict[i.upper()] = char.upper()
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            ...
            count = sum([is_word(valid_words, j) for j in text.split(" ")])
            if count > max_count:
                max_count = count
                end_result = text
                shift = i

        return shift, end_result
...
def decrypt_story():
    ...
    story = encrypted_story.decrypt_message()
